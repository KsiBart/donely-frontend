...
export function HowItWorks() {
  ...
  return (
    <section id="how" className="relative overflow-hidden bg-[var(--howGrad)]">
      <div className="max-w-300 mx-auto w-full box-border p-[clamp(48px,6vw,84px)_22px]">
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(240px,1fr))] gap-6 mt-11">
          {steps.map((s, i) => (
            <div key={s.n} data-reveal={REVEAL_DIRS[i % 3]} className="text-center p-2">
              <div className="w-14.5 h-14.5 mx-auto rounded-full bg-[var(--acc)] text-[var(--onacc)] flex items-center justify-center font-['Bricolage_Grotesque',sans-serif] text-2xl font-extrabold shadow-[0_8px_20px_rgba(122,79,192,.3)]">
                {s.n}
              </div>
              <div className="font-['Bricolage_Grotesque',sans-serif] text-[21px] font-bold text-[var(--bandInk)] mt-4.5">{s.t}</div>
              <div className="text-[15px] text-[var(--bandMuted)] leading-[1.55] mt-2 max-w-80 mx-auto">{s.d}</div>
            </div>
          ))}
        </div>
      </div>
