...
export default function FeaturedCarousel({ t, locale, featured, openProvider }: FeaturedCarouselProps) {
  ...
  return (
    <>
      <div className="flex justify-between items-baseline mt-5.5 mx-5 mb-2.5">
        ...
        <span className="text-[11px] font-bold text-accent bg-surface2 rounded-[9px] py-0.75 px-2">{t('home.featuredBadge')}</span>
      </div>
      <div className="hide-scroll flex gap-2.5 overflow-auto pt-0.5 px-5 pb-1.5">
        {featured.map((p) => (
          <div key={p.id} {...clickable(() => openProvider(p.id))} className="flex-none w-50 bg-surface rounded-[20px] overflow-hidden shadow-[var(--shadow)] cursor-pointer">
            <div
            ...
            >
              <span className="font-semibold text-[9px] font-[ui-monospace,monospace] bg-[rgba(0,0,0,.5)] text-white rounded-[7px] py-0.5 px-1.75">{t('home.workPhoto')}</span>
            </div>
            <div className="pt-2.5 px-3 pb-3">
              ...
              <div className="text-[11.5px] font-bold text-accent mt-1.25">
